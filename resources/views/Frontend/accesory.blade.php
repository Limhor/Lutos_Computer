...
<div style="height: 220vh;">
    <div class="container" style="height:70vh;">
        @foreach($accessories as $acc)
            <div class="card" style="width: 18rem;margin-top: 2%;" id="cards">
                <img class="card-img-top" src="{{asset('/uploads/accessoryimage/' .$acc->Image)}}"  alt="desktop_images" height="250px">
                <div class="card-body">
                    <h5 class="card-title">{{ $acc->Brand_Name }}</h5>
                    <p class="card-text">Prices: {{ $acc->Price }} $</p>
                    <p class="card-text">Type: {{ $acc->Type }} </p>
                    <a href='{{ url("/singleAcc/{$acc->id}") }}' class="btn btn-primary">Go shop</a>
                </div>
            </div>
        @endforeach
            @foreach($accessories as $acc)
