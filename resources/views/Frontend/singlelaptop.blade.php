...
<center>
<form action="{{ url('laptop', array($laptop->id)) }}" method="post" style="border: none;margin-top: 10%;">
    <h2><a href="/laptop" class="text-decoration-none">Laptop</a></h2>
    @csrf
    <div class="card a" style="max-width: 60%;">
        <div class="row no-gutters">
            <div class="col-md-5">
                <img src="{{asset('/uploads/laptopimage/' .$laptop->Laptop_images)}}" width="450px" height="450px" alt="Laptop_images">
            </div>
            <div class="col-md-5">
                <div class="card-body float-right">
                    <div  class="card-text">
                        <h2><b>{{ $laptop->Brand_Name }}</b></h2>
                        <small class="text-warning">&#9733; &#9733; &#9733; &#9733; &#9734;</small>
                    </div>

                    <p class="card-text">Availability: <b>In Stock</b></p>
                    <div class="row">
                        <p class="lead">Model: {{$laptop->Model}}</p>
                    </div>
                    <div class="row">
                        <p class="lead">Brand_Name: {{$laptop->Brand_Name}}</p>
                    </div>
                    <div class="row">
                        <p class="lead">Os: {{$laptop->Os}}</p>
                    </div>
                    <div class="row">
                        <p class="lead">Price: {{$laptop->Price}}$</p>
                    </div>
                    <div class="row">
                        <p class="lead">Processor: {{$laptop->Processor}}</p>
                    </div>
                    <div class="row">
                        <p class="lead">Graphic: {{$laptop->Graphics}}</p>
                    </div>
                    <div class="row">
                        <p class="lead">Display: {{$laptop->Display}}</p>
                    </div>
                    <div class="row">
                        <p class="lead">Storage: {{$laptop->Storage}}</p>
                    </div>
                    <div class="row">
                        <p class="lead">Memory: {{$laptop->Memory}}</p>
                    </div>
                    <div class="row">
                        <p class="lead">Battery: {{$laptop->Battery}}</p>
                    </div>
                    <div class="row">
                        <p class="lead">Keyboard: {{$laptop->Keyboard}}</p>
                    </div>
                </div>
            </div>
        </div>
